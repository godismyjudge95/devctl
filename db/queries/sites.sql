...
-- name: CreateSite :one
INSERT INTO sites (id, domain, root_path, php_version, aliases, spx_enabled, https, cors, auto_discovered, settings, parent_site_id, worktree_branch, public_dir, service_vhost, is_git_repo, git_remote_url, framework)
VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
RETURNING *;

-- name: UpdateSite :one
UPDATE sites
SET domain = ?, root_path = ?, php_version = ?, aliases = ?, spx_enabled = ?, https = ?, cors = ?, settings = ?, public_dir = ?, is_git_repo = ?, git_remote_url = ?, framework = ?, updated_at = datetime('now')
WHERE id = ?
RETURNING *;
