...
func DefaultServices(serverRoot, siteUser string) []services.Definition {
	...
	return []services.Definition{
		...
		},
		{
			ID:                    "postgres",
			Label:                 "PostgreSQL",
			Description:           "PostgreSQL with TimescaleDB (Community Edition)",
			InstallVersion:        "18.4",
			Installable:           true,
			HasCredentials:        true,
			Managed:               true,
		...
		},
		{
			...
			HasCredentials:  true,
			Managed:         true,
			ManagedCmd:      paths.BinDir(serverRoot) + "/php",
			ManagedArgs:     "artisan reverb:start --host=127.0.0.1 --port=7383",
			ManagedDir:      reverbDir,
			ManagedUser:     siteUser,
			CredentialsFile: reverbDir + "/connection.env",
		...
		},
		{
			...
			ManagedArgs:     "--data-dir " + maxioDir + "/data",
			ManagedDir:      maxioDir,
			ManagedEnvFile:  maxioDir + "/config.env",
