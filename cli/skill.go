...
func generateEnvGuide() string {
	...
	sb.WriteString("A separate workspace for one branch is `devctl sites:worktree:add`.\n")
	sb.WriteString("Do not run `git worktree add` yourself.\n")
	sb.WriteString("Do not read the devctl source, binary, or SQLite database.\n\n")
	sb.WriteString("```sh\n")
	sb.WriteString("devctl sites:worktree:add <parent-domain> <branch>\n")
	sb.WriteString("devctl sites:worktree:add <parent-domain> <new-branch> --create\n")
	sb.WriteString("```\n\n")
	sb.WriteString("`<parent-domain>` is the site domain (for example `tools.infomedia.test`), an id, or `.` for the current directory.\n")
	sb.WriteString("`--create` makes the branch from the parent's current HEAD.\n\n")
	sb.WriteString("The new site is a sibling directory.\n")
	sb.WriteString("Its domain is `{parent-dir}-{branch-slug}.test`.\n")
	sb.WriteString("`feature/chatbots` on `tools.infomedia` becomes `https://tools.infomedia-feature-chatbots.test`.\n\n")
	sb.WriteString("`.env` is copied and the hostname is rewritten.\n")
	sb.WriteString("The database stays the parent's database.\n")
	sb.WriteString("`vendor/` and `node_modules/` are copied only when `composer.lock` or the JS lockfile matches the parent.\n")
	sb.WriteString("If either directory is missing, install dependencies in the worktree.\n")
	sb.WriteString("`auth.json` is not copied. Copy it from the parent when Composer asks for credentials.\n\n")
